...
- 当前代码状态：仓库内通用 `rpc_server.py`、Mac `RpcClient` 和 MCP 已统一为 API v1；Windows `5004` 已加载 close detach/timeout 修复，真实 v1 smoke 全流程通过并生成 `.fsp`。`/jobs/*` v1 第一版已实现 plan、start、status、tasks、resume；当前真实执行仅限短 `geometry-smoke`
