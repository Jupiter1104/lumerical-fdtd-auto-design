...
- **Headless/resource 必查**：`clearjobs()` 在 sweep 前；每次 `load()` 后、`save()` 前都要按资源类型设置 `express mode`。CPU 模板/CPU resource 使用 `express mode=0`；GPU 才使用 `express mode=1`。模板和资源不匹配会导致无法仿真。
